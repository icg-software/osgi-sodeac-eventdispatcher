...
import org.sodeac.eventdispatcher.common.edservice.api.IContainerLocation;
...
import org.sodeac.eventdispatcher.common.edservice.impl.ContainerLocation;
...
public class EServiceRegistrationAdapter implements IServiceRegistrationAdapter
{
	public EServiceRegistrationAdapter(IQueue queue)
	{
		super();
	...
	private class ServiceContainer
	{
		...
		private EServiceReference serviceReference = null;
		...
		private void update()
		{
			...
			(
				"sdc://" 
						...
						+ "service" + LDAPFilterEncodingHandler.getInstance().encodeToString
						(
								new Attribute().setName("uuid").setOperator(ComparativeOperator.EQUAL).setValue(this.uuid.toString())
						)
			);
			serviceReference = new EServiceReference(this.uuid, serviceLocation, Collections.unmodifiableList(capabilitiesForReference) );
	...
	{
		List<IEServiceReference> referenceList = null;
		...
		try
		{
			...
			for(ServiceContainer serviceContainer : this.serviceContainerList)
			{
				...
				IEServiceReference serviceReference = serviceContainer.serviceReference;
				...
				try
				{
					...
					{
						referenceList = new ArrayList<IEServiceReference>();
					}
