...
(
	...
	property=
	{
		EventDispatcherConstants.PROPERTY_QUEUE_ID+"="+BasePeriodicJobTestController.QUEUE_ID,
		EventConstants.EVENT_TOPIC+"=" + BasePeriodicJobTestController.SCHEDULE_EVENT
	}
)
public class BasePeriodicJobTestController extends AbstractBaseTestController implements EventHandler,IQueueController,IOnQueuedEvent,IOnRemovedEvent,IOnTaskDone,IOnTaskError,IOnTaskTimeout,IOnFiredEvent,IOnQueueAttach,IOnQueueDetach,IOnQueueSignal
{
	...
	{
		dispatcher.queueEvent(BasePeriodicJobTestController.QUEUE_ID,event);
	}
	...
	public void onQueuedEvent(IQueuedEvent event)
	{
		...
		IQueueTask job = new PeriodicTestJob();
