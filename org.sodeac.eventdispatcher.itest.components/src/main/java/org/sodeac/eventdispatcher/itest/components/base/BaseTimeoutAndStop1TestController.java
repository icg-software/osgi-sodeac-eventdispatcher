...
(
	...
	property=
	{
		EventDispatcherConstants.PROPERTY_QUEUE_ID+"="+BaseTimeoutAndStop1TestController.QUEUE_ID,
		EventConstants.EVENT_TOPIC+"=" + BaseTimeoutAndStop1TestController.SCHEDULE_EVENT
	}
)
public class BaseTimeoutAndStop1TestController extends AbstractBaseTestController implements EventHandler,IQueueController,IOnQueuedEvent,IOnRemovedEvent,IOnTaskDone,IOnTaskError,IOnTaskTimeout,IOnFiredEvent,IOnQueueAttach,IOnQueueDetach,IOnQueueSignal
{
	...
	{
		dispatcher.queueEvent(BaseTimeoutAndStop1TestController.QUEUE_ID, event);
	}
	...
	public void onQueuedEvent(IQueuedEvent event)
	{
		...
		IQueueTask job = new BaseTimeOutAndStop1Job(SLEEP_VALUE);
