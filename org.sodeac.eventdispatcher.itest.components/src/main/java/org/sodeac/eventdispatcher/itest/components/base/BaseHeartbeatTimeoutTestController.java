...
(
	...
	property=
	{
		EventDispatcherConstants.PROPERTY_QUEUE_ID+"="+BaseHeartbeatTimeoutTestController.QUEUE_ID,
		EventConstants.EVENT_TOPIC+"=" + BaseHeartbeatTimeoutTestController.SCHEDULE_EVENT
	}
)
public class BaseHeartbeatTimeoutTestController extends AbstractBaseTestController implements EventHandler,IQueueController,IOnQueuedEvent,IOnRemovedEvent,IOnTaskDone,IOnTaskError,IOnTaskTimeout,IOnFiredEvent,IOnQueueAttach,IOnQueueDetach,IOnQueueSignal
{
	...
	{
		dispatcher.queueEvent(BaseHeartbeatTimeoutTestController.QUEUE_ID,event);
	}
	...
	public void onQueuedEvent(IQueuedEvent event)
	{
		...
		IQueueTask job = new BaseHeartBeatTimeOutJob();
		super.tracingObject.getTracingEventList().add(new TracingEvent(TracingEvent.ON_EVENT_SCHEDULED,event));
		event.getQueue().scheduleTask(null,job,null,-1, -1, HEARTBEAT_TIMEOUT);
	}
}
